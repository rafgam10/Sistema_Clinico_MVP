...
export async function buildGuiaTiss(params: {
  paciente: string
  cpf?: string
  convenio: string
  data: string
  exames: { nome: string, codigo_amb?: string | null, codigo_alfanumerico?: string | null }[]
  medico?: string
  crm?: string
  especialidade?: string
}) {
  const examesContent = params.exames.length
    ? [
        {
          table: {
            headerRows: 1,
            ...
            body: [
              [
                ...
                { text: 'Exame', bold: true, fontSize: 9 },
                { text: 'C\u00F3d. AMB', bold: true, fontSize: 9 },
                { text: 'C\u00F3d. TUSS', bold: true, fontSize: 9 }
              ],
              ...params.exames.map((e, i) => [
                { text: String(i + 1), fontSize: 9 },
                { text: e.nome, fontSize: 9 },
                { text: e.codigo_amb || '\u2014', fontSize: 9 },
                { text: e.codigo_alfanumerico || '\u2014', fontSize: 9 }
              ])
            ]
          },
  ...
  return {
    ...
    content: [
      ...(await hospitalHeader()),
      documentTitle('GUIA TISS \u2014 SOLICITA\u00C7\u00C3O DE EXAMES'),
      { text: 'DADOS DO PACIENTE', bold: true, fontSize: 11, margin: [0, 0, 0, 8] },
      { text: `Nome: ${params.paciente.toUpperCase()}`, margin: [0, 0, 0, 3] },
      ...(params.cpf ? [{ text: `CPF: ${params.cpf}`, margin: [0, 0, 0, 3] }] : []),
      { text: `Conv\u00EAnio: ${params.convenio}`, margin: [0, 0, 0, 15] },
      { text: 'DADOS DO SOLICITANTE', bold: true, fontSize: 11, margin: [0, 0, 0, 8] },
      { text: `M\u00E9dico: ${params.medico ?? 'M\u00E9dico Respons\u00E1vel'}`, margin: [0, 0, 0, 3] },
      ...(params.crm ? [{ text: `CRM: ${params.crm}`, margin: [0, 0, 0, 3] }] : []),
      ...(params.especialidade ? [{ text: `Especialidade: ${params.especialidade}`, margin: [0, 0, 0, 15] }] : []),
      { text: 'EXAMES SOLICITADOS', bold: true, fontSize: 11, margin: [0, 0, 0, 8] },
      ...examesContent,
      { text: `DATA: ${params.data}`, margin: [0, 20, 0, 0] },
      signatureBlock(params.medico, params.crm, params.especialidade)
    ],
    defaultStyle
  }
