export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<{ status: string, consulta?: { anamnese?: string, diagnosticos?: { cid: string, descricao?: string, principal: boolean }[], medicamentos?: string, exames?: string, duracao?: number } }>(event)
