export default defineEventHandler(async (event) => {
  ...
  if (body.status) {
    const validStatuses = ['agendado', 'em-espera', 'em-atendimento', 'atendido', 'faltou', 'cancelado']
    ...
    if (agendamentosPaciente.length > 0) {
      ...
      return atualizarStatusAgendamento(ultimoAgendamento.id, body.status as 'agendado' | 'em-espera' | 'em-atendimento' | 'atendido' | 'faltou' | 'cancelado')
    }
